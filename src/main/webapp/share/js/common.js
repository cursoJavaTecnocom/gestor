// JavaScript Document
	...
	*/
	...
	function iniInputFile(sInputId)
	{
		...
		sConfirmDiv = '<div class="confirmFile"></div>';
		
		//$(".frmContenido").prepend('<div class="confirmFile"></div>');
		//oForm.submit( function() {alert(0);$("#upfilexajax_" + sInputId).remove();});
		//oForm.prepend(sConfirmDiv);
		//		oForm.submit( function() {$("#upfilexajax_" + sInputId).remove();});
		...
		$("#" + sInputId).parent().prepend(sConfirmDiv);
		$("#" + sInputId).remove();

		oContent.append(sNewCode).before('<label><img id="upfilexajax_' + sInputId + 'igmLoadingFile" src="share/img/loading.gif"></label>');
	...
	function ajaxFileUpload(sInputId, sRealInputId)
	{
		...
		*/
		...
		(
			{
				...
				success: function (data, status)
				{
					...
					if(typeof(data.error) != 'undefined')
					{
						if(data.error != '')
						{
							$(".confirmFile").addClass('error').html('<strong>Error</strong><p>' + data.error + '</p>').slideToggle(1).slideToggle(1500, function (){$(".confirmFile").removeClass('error')});
							//$.prompt('<h3>Error</h3><p>' + data.error + '</p>', {prefix:'brownJqi'});
						}
						else
						{
							$('#' + sRealInputId).val(data.file);
							$(".confirmFile").addClass('ok').html('<strong>Archivo subido</strong><p>' + data.msg + '</p>').slideToggle(1).slideToggle(1500, function (){$(".confirmFile").removeClass('ok')});
		...
		$("#" + sInputId).change(function(){return ajaxFileUpload(sInputId);});
		return false;
	...
	function iniInputFilePreview(sInputId)
	{
		...
		oContent.find('img.agregar_imagen').after('<img class="imagen_login" id="upfilexajax_' + sInputId + 'igmLoadingFile" src="http://' + sUrlSite + 'share/img/small_loading.gif">');
		$("#upfilexajax_" + sInputId + "igmLoadingFile").fadeOut(1);
	...
	function menuContenidoTinyMCE (sElement, iAncho, mLinks) 
	{
		tinyMCE.init({
			...
			plugins : "safari,spellchecker,pagebreak,style,layer,table,save,advhr,advimage,advlink,emotions,iespell,inlinepopups,insertdatetime,preview,media,searchreplace,print,contextmenu,paste,directionality,fullscreen,noneditable,visualchars,nonbreaking,xhtmlxtras,template,ajaxfilemanager,table",
//			plugins : "safari,style,autosave,save,advimage,advlink,inlinepopups,preview,media,searchreplace,print,contextmenu,paste,fullscreen,xhtmlxtras,ajaxfilemanager",
...
/*
			...
			width: iAncho + 'px',
*/
			theme_advanced_buttons1 : "newdocument,|,bold,italic,underline,strikethrough,|,justifyleft,justifycenter,justifyright,justifyfull,|,formatselect,fontselect,fontsizeselect,|,media,|,print,|,fullscreen,|,preview|,search,replace,|,bullist,numlist",
			theme_advanced_buttons2 : "pastetext,pasteword,|,outdent,indent,blockquote,|,undo,redo,|,link,unlink,anchor,image,cleanup,code,|,forecolor,backcolor,|,tablecontrols,hr",
			theme_advanced_buttons3 : "",
			//theme_advanced_buttons4 : "youtube",
			//theme_advanced_buttons4 : "insertlayer,moveforward,movebackward,absolute,|,styleprops,spellchecker,|,cite,abbr,acronym,del,ins,attribs,|,visualchars,nonbreaking,template,blockquote,pagebreak,|,insertfile,insertimage",
			theme_advanced_toolbar_location : "top",
			theme_advanced_toolbar_align : "left",
			theme_advanced_statusbar_location : "bottom",
		...
		function ajaxfilemanager(field_name, url, type, win) {
			var ajaxfilemanagerurl = "http://"+sUrlSite+"/share/js/tiny_mce/plugins/ajaxfilemanager/ajaxfilemanager.php?editor=tinymce";
			switch (type) {
				case "image":
					break;
				case "media":
			...
			tinyMCE.activeEditor.windowManager.open({
				url: "http://"+sUrlSite+"/share/js/tiny_mce/plugins/ajaxfilemanager/ajaxfilemanager.php?editor=tinymce",
				width: 782,
				height: 440,
				inline : "yes",
				close_previous : "no"
