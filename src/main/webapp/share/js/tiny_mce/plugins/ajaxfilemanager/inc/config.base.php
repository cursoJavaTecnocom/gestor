...
//error_reporting(E_ALL ^ E_NOTICE);	
	...
	//FILESYSTEM CONFIG
		...
		*/
/*
	$aForderPath = explode('/', $_SERVER['PHP_SELF']);
	$sPath = "{$_SERVER['DOCUMENT_ROOT']}/{$aForderPath[1]}/{$aForderPath[2]}";
*/
	$aForderPath = explode('/', $_SERVER['PHP_SELF']);
	$sPath = "{$_SERVER['DOCUMENT_ROOT']}";
	foreach ($aForderPath as $iPos => $sValue)
	{
		if ($sValue == 'administrador') break;
		$sPath .= "/{$aForderPath[$iPos]}";
	}

	define('CONFIG_SYS_DEFAULT_PATH', $sPath . '/uploaded/'); //accept relative path only
	define('CONFIG_SYS_ROOT_PATH', $sPath . '/uploaded/');	//accept relative path only
	...
	define('CONFIG_UPLOAD_VALID_EXTS', 'gif,jpg,png,txt,swf,flv,doc,pdf,docx'); //
